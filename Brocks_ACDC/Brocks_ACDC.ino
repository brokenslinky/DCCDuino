...
void loop() {
  // First check if user is providing any input.
  check_user_input();

  // Check the speedometer signal every iteration to make sure no pulses are missed.
  // checkSpeedo(); Not used in current implementation

  // Time needs to be tracked for both speed and angle measurements.
  // Currently unused.
  // time = micros();
